        ...
        <form wire:submit="update" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                ...
                <div>
                    ...
                    <div class="border border-dashed border-gray-300 rounded-lg p-8 flex items-center justify-center bg-gray-50 min-h-[200px]">
                        @if($currentImage)
                            @php
                                $imageSrc = filter_var($currentImage, FILTER_VALIDATE_URL) 
                                    ? $currentImage 
                                    : asset($currentImage);
                            @endphp
                            <img id="currentImage" 
                    ...
                    <div class="border border-dashed border-gray-300 rounded-lg p-8 bg-gray-50 min-h-[200px] flex flex-col justify-center">
                        <div class="flex items-center gap-4">
                            <label for="imageUpload" class="px-4 py-2 bg-blue-50 text-blue-600 text-sm font-medium rounded-lg cursor-pointer hover:bg-blue-100 transition-colors">
                                <span wire:loading.remove wire:target="image_file">Choose File</span>
                                <span wire:loading wire:target="image_file">Uploading...</span>
                            </label>
                        ...
                        @if ($image_file) 
                            <div class="mt-4">
                                <img src="{{ $image_file->temporaryUrl() }}" alt="Preview" class="max-w-full h-32 object-contain rounded-lg border border-gray-200">
                            </div>
                        @endif
            ...
            <div>
                ...
                <textarea id="description" 
                          ...
                          rows="4"
                          class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white text-gray-900 text-sm"></textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-100">
                ...
                <button type="submit" 
                        ...
                        class="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm disabled:opacity-50">
